...
type ElementTree = Rc<ElementNode>;

struct ElementNode {
    element: Element,
    children: Vector<ElementTree>,
    focus: Option<usize>,
}
...
struct Callbacks {
    mouse_move: Option<Box<Fn(Point)>>,
    mouse_down: Option<Box<Fn(MouseButton)>>,
    mouse_up: Option<Box<Fn(MouseButton)>>,
    mouse_scroll: Option<Box<Fn(f32)>>,
    key_down: Option<Box<Fn(KeyboardButton)>>,
    key_up: Option<Box<Fn(KeyboardButton)>>,
    text_input: Option<Box<Fn(char)>>,
}
...
type LayoutTree = Rc<LayoutNode>;

#[derive(Clone)]
struct LayoutNode {
    bounding_box: BoundingBox,
    data: ElementLayoutData,
    children: Vector<LayoutTree>,
}

#[derive(Clone)]
enum ElementLayoutData {
    Rect,
    Text(Vector<PositionedGlyph<'static>>),
}
...
pub enum InputEvent {
    ...
    MouseUp(MouseButton),
    MouseScroll(f32),
    KeyDown(KeyboardButton),
    KeyUp(KeyboardButton),
    TextInput(char),
}
...
impl UI {
    pub fn new(width: f32, height: f32) -> UI {
        ...
        UI {
            ...
            root: Rc::new(ElementNode { element: Element { display: Display::Rect([0.0, 0.0, 0.0, 0.0]), layout: Layout::default(), callbacks: Callbacks::default() }, children: Vector::new(), focus: None }),
            layout: Rc::new(LayoutNode { bounding_box: BoundingBox::new(0.0, 0.0, 0.0, 0.0), data: ElementLayoutData::Rect, children: Vector::new() }),
    ...
    pub fn root(&mut self, tree: ElementTree) {
        self.root = tree;
        self.layout = self.layout(&self.root);
    }
    ...
    pub fn input(&mut self, event: InputEvent) {
        ...
        match event {
            InputEvent::MouseMove(..) | InputEvent::MouseDown(..) | InputEvent::MouseUp(..) | InputEvent::MouseScroll(..) => {
                let position = self.input_state.mouse_drag_origin.unwrap_or(self.input_state.mouse_position);
                dispatch_mouse_event(&self.root, &self.layout, position, event);
            }
            InputEvent::KeyDown(..) | InputEvent::KeyUp(..) | InputEvent::TextInput(..) => {
                dispatch_key_event(&self.root, &self.layout, event);
            }
        }
    ...
    pub fn display(&self) -> DisplayList {
        ...
        self.display_tree(&self.root, &self.layout, &mut list);
        list
    }

    fn display_tree(&self, tree: &ElementTree, layout: &LayoutTree, list: &mut DisplayList) {
        self.display_element(&tree.element, layout.bounding_box, list);
        for (child, child_layout) in tree.children.iter().zip(layout.children.iter()) {
            self.display_tree(child, child_layout, list);
        }
    }

    fn display_element(&self, element: &Element, bounding_box: BoundingBox, list: &mut DisplayList) {
        match element.display {
            Display::Rect(color) => {
                list.rect(Rect {
                    x: bounding_box.pos.x,
                    y: bounding_box.pos.y,
                    w: bounding_box.size.x,
                    h: bounding_box.size.y,
                    color: color,
                });
            }
            Display::Text(ref text) => {
                let glyphs = layout_text(text, Scale::uniform(14.0), &self.font, bounding_box.pos);
    ...
    pub fn layout(&self, tree: &ElementTree) -> LayoutTree {
        Rc::new(LayoutNode { bounding_box: BoundingBox::new(0.0, 0.0, 0.0, 0.0), data: ElementLayoutData::Rect, children: Vector::new() })
    }
...
fn dispatch_mouse_event(tree: &ElementTree, layout: &LayoutTree, position: Point, event: InputEvent) -> bool {
    if layout.bounding_box.contains_point(position) {
        for (child, child_layout) in tree.children.iter().zip(layout.children.iter()) {
            if dispatch_mouse_event(child, child_layout, position, event) {
                return true;
            }
        }
        match event {
            InputEvent::MouseMove(point) => {
                if let Some(ref mouse_move) = tree.element.callbacks.mouse_move {
                    mouse_move(point);
                    return true;
                }
            }
            InputEvent::MouseDown(button) => {
                if let Some(ref mouse_down) = tree.element.callbacks.mouse_down {
                    mouse_down(button);
                    return true;
                }
            }
            InputEvent::MouseUp(button) => {
                if let Some(ref mouse_up) = tree.element.callbacks.mouse_up {
                    mouse_up(button);
                    return true;
                }
            }
            InputEvent::MouseScroll(delta) => {
                if let Some(ref mouse_scroll) = tree.element.callbacks.mouse_scroll {
                    mouse_scroll(delta);
                    return true;
                }
            }
...
fn dispatch_key_event(tree: &ElementTree, layout: &LayoutTree, event: InputEvent) -> bool {
    if let Some(focus) = tree.focus {
        if dispatch_key_event(&tree.children[focus], &layout.children[focus], event) {
            return true;
        }
    }
    match event {
        InputEvent::KeyDown(button) => {
            if let Some(ref key_down) = tree.element.callbacks.key_down {
                key_down(button);
                return true;
            }
        }
        InputEvent::KeyUp(button) => {
            if let Some(ref key_up) = tree.element.callbacks.key_up {
                key_up(button);
                return true;
            }
        }
        InputEvent::TextInput(character) => {
            if let Some(ref text_input) = tree.element.callbacks.text_input {
...
fn layout_text(text: &Cow<'static, str>, scale: Scale, font: &Font, pos: Point) -> Vec<PositionedGlyph<'static>> {
